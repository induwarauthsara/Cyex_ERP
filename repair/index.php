...
if (isset($con)) {
    ...
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            array_push($bankAccountsOptionList, '<option value="' . $row['emp_name'] . '"> ' . $row['emp_name'] . ' </option>');
        }
    }
}
?>
    ...
    <div class="main">
        <div class="bill">
            ...
            <div class="billHeadButtons">
                <a href="addNewRepair.php"></a> <button class="add_pettycash"> Add New Repair Item </button>
                <a href="repairCategoryList.php"></a> <button class="add_pettycash"> Add New Repair Category</button>
            </div>
        ...
        <form action="submit.php" method="POST">
            ...
            <div class="content">
                <!--tabel-->
                <textarea name="InvoiceDescription" id="InvoiceDescription" placeholder="Add Details about this Phone Repair, Damages, Passwords"></textarea>
                <div class="container">
                    <!--Tabel Head Start-->
                    <div class="Product tabel-head">Repair Name</div>
                    <div class="QTY tabel-head">Cost</div>
                    <div class="Rate tabel-head">Employee Commission</div>
                    <div class="Amount tabel-head">Selling Price</div>
                    <!--Tabel Head End-->
                    <div class="product_list tabel" id="Product">
                        <div id="list"></div>
                        <hr>
                        <input list="products" type="text" id="addproduct">
                        <div onclick="addRepair()" class="add">Add </div>
                    </div>
...
<datalist id="products">
    <?php $product_list = "SELECT repair_name FROM repair_items";
    $result = mysqli_query($con, $product_list);
    if ($result) {
        echo "<ol>";
        while ($recoard = mysqli_fetch_assoc($result)) {
            $product = $recoard['repair_name'];
